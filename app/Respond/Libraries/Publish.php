...
class Publish
{
    ...
    public static function publishTheme($theme, $site)
    {
        ...
        $src_basepath = realpath(APP_ROOT . '/public/' . getenv('THEMES_LOCATION'));
        $dest_basepath = realpath(APP_ROOT . '/public/sites');
        ...
        // new dest_basepath
        $dest_basepath = realpath(APP_ROOT . '/resources/sites/');
    ...
    public static function updatePlugins($site)
    {
      ...
      $src = APP_ROOT . '/public/' . getenv('THEMES_LOCATION') . $site->theme . '/plugins';
      $dest = APP_ROOT . '/public/sites/' . $site->id . '/plugins';

      // copy the directory
      Utilities::copyDirectory($src, $dest);

      // copy the plugin JSON file
      $src = APP_ROOT  . '/public/' . getenv('THEMES_LOCATION') . $site->theme . '/data/plugins.json';
      $dest = APP_ROOT . '/public/sites/' . $site->id . '/data/plugins.json';

      copy($src, $dest);

      // copy the JS file
      $src = APP_ROOT  . '/public/' . getenv('THEMES_LOCATION') . $site->theme . '/js/plugins.js';
      $dest = APP_ROOT . '/public/sites/' . $site->id . '/js/plugins.js';

      copy($src, $dest);

      // copy the CSS file
      $src = APP_ROOT  . '/public/' . getenv('THEMES_LOCATION') . $site->theme . '/css/plugins.css';
      $dest = APP_ROOT . '/public/sites/' . $site->id . '/css/plugins.css';
    ...
    public static function combineCSS($site)
    {
      ...
      // set dir
      $dir = APP_ROOT.'/public/sites/'.$id.'/css';
    ...
    public static function combineJS($site)
    {
      ...
      // set dir
      $dir = APP_ROOT.'/public/sites/'.$id.'/js';
      ...
      // check for libs
      $libs_file = APP_ROOT.'/public/sites/'.$id.'/js/libs.min.js';
      ...
      // check for libs
      $plugins_file = APP_ROOT.'/public/sites/'.$id.'/js/plugins.js';
    ...
    public static function publishSiteMap($user, $site)
    {
      ...
      // xml file
      $file = APP_ROOT . '/public/sites/' . $site->id . '/sitemap.xml';
    ...
    public static function migrate($user, $site)
    {
      ...
      foreach($pages as $page) {
        ...
        // get html of page
        $file = APP_ROOT . '/public/sites/' . $site->id . '/' . $url . '.html';
    ...
    public static function publishSettingsForPage($page, $user, $site) {
      ...
      // get settings
      $file = APP_ROOT.'/resources/sites/'.$site->id.'/settings.json';

      $settings = json_decode(file_get_contents($file), true);


      $path = APP_ROOT.'/public/sites/'.$site->id.'/'.$page->url.'.html';
    ...
    public static function publishTemplates($user, $site) {
      ...
      foreach($files as $file) {
        ...
        $path = APP_ROOT.'/public/sites/'.$site->id.'/'.$file;
    ...
    public static function publishTemplate($template, $user, $site) {
      ...
      $template_file = $dir = APP_ROOT.'/public/sites/'.$site->id.'/templates/'.$template.'.html';
      ...
      if(file_exists($template_file)) {
        ...
        foreach($pages as $item) {
          ...
          if($page->template == $template) {
            ...
            // get html of page
            $page_file = APP_ROOT . '/public/sites/' . $site->id . '/' . $page_url . '.html';
    ...
    public static function publishPluginsForPage($page, $user, $site, $pages = null)
    {
      ...
      // get html of page
      $location = APP_ROOT . '/public/sites/' . $site->id . '/' . $filename . '.html';
    ...
    public static function injectPluginHTML($html, $user, $site, $meta, $pages = null) {
      ...
      $dir = APP_ROOT.'/public/sites/'.$site->id.'/plugins/';
      $exts = array('html', 'php');
      ...
      $template_dirs = array(
        APP_ROOT.'/public/sites/'.$site->id.'/plugins'
      );

      $local_plugin_dir = APP_ROOT.'/public/sites/'.$site->id.'/plugins';
      $global_plugin_dir = APP_ROOT.'/resources/plugins';
