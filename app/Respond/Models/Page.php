...
class Page {
  ...
  public static function add($data, $site, $user, $content = NULL){
    ...
    $dest = APP_ROOT.'/public/sites/'.$site->id;
    ...
    if($content == NULL) {
      ...
      // get template
      $template_file = APP_ROOT.'/public/sites/'.$site->id.'/templates/'.$page->template.'.html';
      ...
      if(file_exists($template_file)) {
        ...
        // get default content
        $default_content = APP_ROOT.'/public/sites/'.$site->id.'/.default.html';
    ...
    $json_file = APP_ROOT.'/public/sites/'.$site->id.'/data/pages.json';
  ...
  public static function edit($url, $changes, $site, $user){
    ...
    // get page
    $location = APP_ROOT.'/public/sites/'.$site->id.'/'.$url.'.html';
  ...
  public function remove($user, $site){
    ...
    $page_location = APP_ROOT.'/public/sites/'.$site->id.'/'.$this->url.'.html';
  ...
  public function injectTemplate($page_html, $site, $user) {
    ...
    $template_file = $dir = APP_ROOT.'/public/sites/'.$site->id.'/templates/'.$this->template.'.html';
  ...
  public function save($site, $user) {
    ...
    // set full file path
    $file = APP_ROOT . '/public/sites/' . $site->id . '/' . $this->url;
  ...
  public static function getByUrl($url, $id){
    ...
    $file = APP_ROOT.'/public/sites/'.$id.'/data/pages.json';
  ...
  public static function listExtended($user, $site){
    ...
    $base_url = APP_ROOT.'/public/sites/'.$site->id.'/';
  ...
  public static function listAllBySite($siteId){
    ...
    // get base path for the site
    $json_file = APP_ROOT.'/public/sites/'.$siteId.'/data/pages.json';
  ...
  public static function refreshJSON($user, $site) {
    ...
    // set dir
    $dir = APP_ROOT.'/public/sites/'.$site->id;
    ...
    foreach ($files as $file) {
        ...
        // set full file path
        $file = APP_ROOT . '/public/sites/' . $site->id . '/' . $file;
        ...
        if(isset($els[0])) {
          ...
          if($els[0]->getAttribute('data-template') !== FALSE) {
            ...
            $template = APP_ROOT.'/public/sites/'.$site->id.'/templates/default.html';
        ...
        if ($photo === NULL || $photo === '') {
          ...
          $thumb_file = APP_ROOT . '/public/sites/' . $site->id . '/'.$thumb;
