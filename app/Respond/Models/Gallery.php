...
class Gallery {
  ...
  public static function listAll($siteId) {
    ...
    $dir = APP_ROOT.'/public/sites/'.$siteId.'/data/galleries/';
    $exts = array('json');
    ...
    if(file_exists($dir)) {
      ...
      foreach($files as $file) {
        ...
        $path = APP_ROOT.'/public/sites/'.$siteId.'/'.$file;
  ...
  public static function getById($id, $siteId) {
    ...
    $file = APP_ROOT.'/public/sites/'.$siteId.'/data/galleries/'.$id.'.json';
  ...
  public static function add($name, $siteId) {
    ...
    $x = 1;
    $folder = APP_ROOT.'/public/sites/'.$siteId.'/data/galleries/'.$id.'.json';

    while(file_exists($folder) === TRUE) {

      // increment id and folder
      $new_id = $id.$x;
      $folder = APP_ROOT.'/public/sites/'.$siteId.'/data/galleries/'.$new_id.'.json';
      $x++;
    ...
    // get file
    $dir = APP_ROOT.'/public/sites/'.$siteId.'/data/galleries/';
    $file = APP_ROOT.'/public/sites/'.$siteId.'/data/galleries/'.$new_id.'.json';
  ...
  public function save($siteId) {
    ...
    // get file
    $file = APP_ROOT.'/public/sites/'.$siteId.'/data/galleries/'.$this->id.'.json';
