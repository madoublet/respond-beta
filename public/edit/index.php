<?php
  ...
  $m = filter_var($_GET['mode'], FILTER_SANITIZE_STRING);
  ...
  if($q != NULL){
    ...
    if(sizeof($arr) > 0) {
      ...
      // load page
      $path = APP_ROOT.'/public/sites/'.$url;
      $path = rtrim($path, '/');
      ...
      if(file_exists($path)) {
        ...
        if($mode == 'focused') {
          ...
          $default_template_location = APP_ROOT.'/public/html/index.html';
        ...
        if(isset($el) == false) {
          ...
          if(isset($arr[0])) {
            ...
            $default_template_location = APP_ROOT.'/public/sites/'.$id.'/templates/default.html';
        ...
        // get custom plugins
        $json_file = APP_ROOT.'/public/sites/'.$siteId.'/data/plugins.json';
        ...
        if(file_exists($json_file)) {
          ...
          $js_file = APP_ROOT.'/resources/sites/'.$siteId.'/plugins.js';
        ...
        if(strpos($plugins_script, 'respond.components.routes') !== false ) {
          ...
          $dir = $file = APP_ROOT.'/public/sites/'.$siteId.'/components';
        ...
        if(strpos($plugins_script, 'respond.routes') !== false ) {
          ...
          $dir = $file = APP_ROOT.'/public/sites/'.$siteId;
...
login: '/login/$siteId',
translate: true,
stylesheet: ['/dev/hashedit/css/hashedit.css', '/api/editor/css.php'],
...
stylesheet: ['/app/libs/hashedit/dist/hashedit-min.css', '/api/editor/css.php'],
translate: true,
languagePath: '/assets/i18n/{{language}}.json',
auth: 'token',
authHeader: 'X-AUTH',
